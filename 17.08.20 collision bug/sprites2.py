...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        hitList = pygame.sprite.spritecollide(self, self.currentLevel.layers[MAP_COLLISION_LAYER].collision_tiles, False)
...
#tile map is passed into the level class
...
class Level(object):
    ...
    def shiftLevel(self, shiftX):
        ...
        for layer in self.layers:
            for tile in layer.background_tiles:
                tile.rect.x += shiftX
            for tile in layer.collision_tiles:
                tile.rect.x += shiftX
            for tile in layer.coin_tiles:
                tile.rect.x += shiftX
...
class Layer(object):
    def __init__(self, index, mapObject):
        ...
        self.coin_image = pygame.image.load('resources/coin.png').convert_alpha()
        self.coin_image = pygame.transform.scale(self.coin_image, (64, 64))
            ...
            if isinstance(layer, pytmx.TiledTileLayer):
                if layer == "background":
                    for x, y, tile in layer.tiles():
                        if tile:
                            self.background_tiles.add(Tile(image=tile, x=(x * self.mapObject.tilewidth), y=(y * self.mapObject.tileheight)))
            elif isinstance(layer, pytmx.TiledTileLayer):
                if layer == "collision":
                    for x, y, tile in layer.tiles():
                        if tile:
                            self.collision_tiles.add(Tile(image=tile, x=(x * self.mapObject.tilewidth), y=(y * self.mapObject.tileheight)))
            elif isinstance(layer, pytmx.TiledObjectGroup):
                if layer == "coin":
                    for obj in layer:
                        self.coin_tiles.add(Tile(image=self.coin_image, x=object.x, y=object.y))
    ...
    # Draw layer
    def draw(self, screen):
        self.background_tiles.draw(screen)
        self.collision_tiles.draw(screen)
